...
func newBackend(bcfg BackendConfig) *backend {
	...
	//===注意这个选项,是boltdb的一个选项====
	//1.设置这个选项会导致 db在每次commit的时候跳过fsync()调用,不要设置!!
...
/**
====每次读请求,都会!!复制!!!一个backend.readTx.txReadBuffer=====
*/
// ConcurrentReadTx creates and returns a new ReadTx, which:
// A) creates and keeps a copy of backend.readTx.txReadBuffer,
// B) references the boltdb read Tx (and its bucket cache) of current batch interval.
func (b *backend) ConcurrentReadTx() ReadTx {
	//===对backend.readTx加读锁,每个读请求都会加读取,不会相互阻塞=====
	b.readTx.RLock()
	...
	return &concurrentReadTx{
		baseReadTx: baseReadTx{
			//buf代表一个读事务的一个缓存,后面在查找的时候可以看到优先使用这个缓存
...
//backend定时任务,时间间隔默认是0,可启动时传入 backend-batch-interval,代表了事务从提价到刷入boltdb最长的时间间隔.
func (b *backend) run() {
	defer close(b.donec)
